'''PAST: Process-Aware Spatio-Temporal graph-neural-networks'''
import torch
...
class TGCNmodel(nn.Module):
    ...
    def forward(self, rainfall,inflow, device):
        ...
        for t in range(seq_len):
            ...
            current_runoff[:,753] = current_runoff[:,753] + inflow[:, t, 0] # 753 is the number of the inflow node
...
def valid(model, val_loader, device):
    ...
    total_val_loss = 0
    with torch.no_grad():
        for val_rainfall, val_inflow, val_labels in val_loader:
            val_rainfall = val_rainfall.to(device).to(torch.float32)
            val_inflow = val_inflow.to(device).to(torch.float32)
            val_labels = val_labels.to(device).to(torch.float32)
            val_predictions, _, _= model(val_rainfall, val_inflow, device)
...
def Train(model, train_loader, val_loader, epochs, optimizer, scheduler, device, save_path, patience=20, min_delta=1e-4):
    ...
    for epoch in range(epochs):
        ...
        for rainfall, inflow, lat_labels, labels in train_loader:
            rainfall = rainfall.to(device).to(torch.float32)
            inflow = inflow.to(device).to(torch.float32)
            lat_labels = lat_labels.to(device).to(torch.float32)
            labels = labels.to(device).to(torch.float32)

            optimizer.zero_grad()
            predictions, lateral, _ = model(rainfall, inflow, device)
            loss = criterion(predictions, labels) + criterion(lateral, lat_labels)
