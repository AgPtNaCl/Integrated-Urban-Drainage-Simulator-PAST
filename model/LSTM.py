...
class TGCNmodel(nn.Module):
    ...
    def forward(self, rainfall,inflow, device):
        ...
        runoff = F.relu(self.fc(lstm_out)) 
        for t in range(seq_len):
            ...
            x_gcn = x.contiguous()
            x_gcn = self.conv1(x_gcn)
            x_gcn = F.leaky_relu(x_gcn) 
            x_gcn = self.conv2(x_gcn)
            x_gcn = F.leaky_relu(x_gcn) 
